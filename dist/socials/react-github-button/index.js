...
var propTypes = {
    ...
    type: _react2.default.PropTypes.oneOf(['star', 'fork', 'watch', 'issue', 'issueAll', 'download', 'follow']),
...
function GithubButton(props) {
    ...
    var userRepo = '' + props.user + (props.repo ? '/' + props.repo : '');
    var githubSrc = 'https://buttons.github.io/buttons.html#' + ('href=https://github.com/' + userRepo + TYPES[props.type].href) + ('&text=' + TYPES[props.type].label) + ('&data-icon=' + TYPES[props.type].icon) + ('&data-count-api=/repos/' + userRepo + TYPES[props.type].api) + ('&data-count-href=/' + userRepo + TYPES[props.type].countHref) + ('&data-count-aria-label=' + TYPES[props.type].countLabel) + ('' + (props.large ? '\n&data-style=mega' : '')) + ('&aria-label=' + TYPES[props.type].label + ' ' + userRepo + ' on Github');
